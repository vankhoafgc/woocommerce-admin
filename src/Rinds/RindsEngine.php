...
class RindsEngine {
	...
	private static function run_spec( $spec, &$meta ) {
		...
		$note->set_title( __( $locale->title, 'woocommerce-admin' ) );
		$note->set_content( __( $locale->content, 'woocommerce-admin' ) );
		...
		foreach ( $spec->actions as $action ) {
			...
			$note->add_action(
				$action->name,
				__( $action_locale->label, 'woocommerce-admin' ),
	...
	private static function get_processor( $rule_type ) {
		...
		throw new \WC_Data_Exception( 'invalid_rinds_rule_type', __( 'Rule type could not be found', 'woocommerce' ));
	}
	...
	private static function get_locale( $locales ) {
		...
		throw new \WC_Data_Exception( 
			'invalid_rinds_spec_locale', 
			__( 'Matching locale or fallback en_US locale not found. Make sure there is at least a en_US locale provided.', 'woocommerce-admin' )
		);
	}
	...
	private static function get_action_locale( $action_locales, $note_locale ) {
		...
		throw new \WC_Data_Exception(
			'invalid_rinds_spec_action_locale',
			__( 'Matching locale or fallback en_US locale not found. Make sure there is at least a en_US locale provided.', 'woocommerce-admin' )
		);
	}
}
